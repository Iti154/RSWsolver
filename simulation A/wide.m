...
Omega = 2 * pi / 86400;
R_earth = 2.723945245347076e+06;
g = 9.81;

%% Solver parameters

lat = 45;                          % Mid-latitude (deg)
Lx = 20000;                           % Channel length (m)
Ly_norm = 60;                          % Channel width (Rd)
...
gifname = 'A_wide.gif';   % Name of gif file
dataname = 'A_wide.mat';  % Name of data file
...
dx = 1e4;                          % Spatial resolution x-axis (m)
dy = L/100;                          % Spatial resolution y-axis (m)
dt = T/1000;                            % Time step
...
N = time_f*T/dt;                        % Number of iterations
plot_rate = T/dt/10;        % sampling every plot_rate iterations
...
if (save_data)
    ...
    dataf.H = zeros(ny - 2, data_logs);
    dataf.U = zeros(ny - 2, data_logs);
    dataf.V = zeros(ny - 2, data_logs);
    dataf.y = y(j)/L;
end
...
for t = 0 : N
    ...
    if (mod(t, plot_rate) == 0) || t == N
        cla;
        uu = u*H(nx/2, j, tn);
        plot(y(j) / L, (H(nx/2, j, tn) - H0)','b')
        hold on;
        plot(y(j) / L, U(nx/2, j, tn)./uu,'r')
        plot(y(j) / L, V(nx/2, j, tn)./uu,'g');
        ...
        if (save_data)
            ...
            dataf.H(:, data_idx) = H(nx/2, j, tn)'-H0;
            dataf.U(:, data_idx) = U(nx/2, j, tn)'./uu';
            dataf.V(:, data_idx) = V(nx/2, j, tn)'./uu';
            data_idx = data_idx + 1;
